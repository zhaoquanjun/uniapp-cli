...
export const FXQ = {
  //签章颜色定义
  colors: ['#EC0400', '#2527FA', '#272727'],
  //签章字体定义
  fonts: ['"STSong", "NSimSun"', 'YouYuan', 'KaiTi', 'STHeiti'],
  baseConf: {
    color: null,
    font: null
  },
   ...
   * @param fType 字体 0.宋体
   * @param sType 签章类型 string
   * @param seaNo  这个是签章的序列号
   */
  companySeal: function (company, cType, fType, sType, seaNo, callback) {
    //获取canvans doc对象
    // var canvas = document.getElementById('canvas');
    var canvas = uni.createSelectorQuery().select('#myCanvas');
    var context = uni.createCanvasContext('myCanvas');
    canvas.width = 300;
    canvas.height = 300;
    this.commonMethod(cType, fType);
    var width = canvas.width / 2;
    var height = canvas.height / 2; //画圆方法
      ...
      uni.canvasToTempFilePath({
        ...
        success: res => {
          uni.getFileSystemManager().readFile({
            ...
            success: res => {
              //成功的回调
              typeof callback == 'function' && callback('data:image/png;base64,' + res.data);
            }
          });
        },
        fail: err => {
    ...
    function drawCircle() {
      ...
      context.beginPath(); // context.arc(width,height,110,0,Math.PI*2);

      context.arc(width, height, 145, 0, Math.PI * 2);
      context.stroke();
      context.save(); // var data = canvas.toDataURL("image/jpeg")
    }
    /**
     * 绘制五角星
     * @param sx
    ...
    function draw5Start() {
      context.save();
      context.fillStyle = FXQ.baseConf.color; //移动坐标原点  中心点开始绘制

      context.translate(width, height); //旋转
      ...
      for (var i = 0; i < 5; i++) {
        ...
        var y = Math.cos(i * dig); // context.lineTo(x*30,y*30);

        context.lineTo(x * 45, y * 45);
      }
    ...
    function drawTitle() {
      ...
      if (title.length > 7) {
        fontSize = 22 * (7 / title.length);
      } // 绘制印章名称


      context.font = fontSize + 'px ' + FXQ.baseConf.font;
      ...
      context.fillStyle = FXQ.baseConf.color; // context.fillText(title,width,height+60);

      context.fillText(title, width, height + 80); // let titleArr = title.split('');
      // let fontSpace = 200 / (titleArr.length * 20);
      // for (let i = 0; i < titleArr.length; i++) {
      //     context.fillText(titleArr[i],width,height+60);
      // }
    }
    /**
     * 绘制公司名称
    ...
    function writeFont() {
      // context.font = '900 30px '+FXQ.baseConf.font;
      context.font = '900 42px ' + FXQ.baseConf.font;
      ...
      for (var i = 0; i < count; i++) {
        ...
        if (i == 0) context.rotate(5 * Math.PI / 6.1);else context.rotate(angle);
        context.save(); // context.translate(90, 0);// 平移到此位置,此时字和x轴垂直，第一个参数是与圆外边的距离，越大距离越近

        context.translate(120, 0); // 平移到此位置,此时字和x轴垂直，第一个参数是与圆外边的距离，越大距离越近
    ...
    function writeSerNo(serNo) {
      context.translate(width, height); // context.font = '14px '+FXQ.baseConf.font;

      context.font = '18px ' + FXQ.baseConf.font;
      ...
      for (var i = 0; i < count; i++) {
        ...
        if (i == 0) context.rotate(10 * Math.PI / 5.95);else context.rotate(angle);
        context.save(); // context.translate(-90, 5);// 平移到此位置,此时字和x轴垂直，第一个参数是与圆外边的距离，越大距离越近

        context.translate(-120, 5); // 平移到此位置,此时字和x轴垂直，第一个参数是与圆外边的距离，越大距离越近
   ...
   * @param type 类型
   */
  personal(name, cType, fType, type, border, callback) {
    this.commonMethod(cType, fType);
    ...
    console.log(context, canvas); //长方形前面

    if (type == 0) {
      //要边框
      if (border == 0) {
        // context.rect(5, 5, 100, 50);
        context.rect(10, 35, 100, 50);
      ...
      personal = names[2] + names[0] + names[3] + names[1]; //要边框

      if (border == 0) {
        // context.rect(2,2,108,108);
        context.rect(6, 6, 108, 108);
        context.strokeStyle = color;
        context.lineWidth = 2;
        context.stroke();
      } // 绘制印章名称
      ...
      context.fillStyle = color; // this.wrapText(context,personal,8,45,80,40)

      this.wrapText(context, personal, 15, 50, 80, 40);
    }
  ...
  companyEllipse: function (company, serNo, fType, cType) {
    //椭圆长轴半径
    var radiusX = 140; //短轴半径

    var radiusY = 90;
    this.commonMethod(cType, fType);
    var color = FXQ.baseConf.color;
    var font = FXQ.baseConf.font; //var canvas = document.getElementById('canvas');
    ...
    function writeFont(isTop, words) {
      ...
      var radiusHeight = radiusY + context.lineWidth; //从边线向中心的移动因子
    ...
    function drawEllipse() {
      context.ellipse(radiusX + context.lineWidth + 1, radiusY + context.lineWidth + 1, radiusX, radiusY, 0, 0, Math.PI * 2); //背景透明
