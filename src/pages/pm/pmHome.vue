<template>
  ...
  <view>
        ...
        <view class="person-box" v-if="userToken">
          ...
          <view class="info-box">
            <view class="userName">
              ...
              <view class="auth-btn" @tap.stop="authAction" v-if="isAuth * 1"
                >已认证</view
              >
              <view class="auth-btn" @tap.stop="authAction" v-if="!isAuth * 1"
                >未认证</view
          ...
          <view v-for="(item, index) in items" :key="index">
            ...
            >
              <image
                ...
                mode="aspectFit"
              ></image>
              <view class="user-content-cell-text global-title-color">{{
                item.name
              }}</view>
    ...
    ></slider-picker>
    <button v-if="!userToken" class="login-btn" @tap.stop="login">
      立即登录
    </button>
    <wechatUserInfo
      :show="wechatUserInfoShow"
  ...
  data() {
    return {
      items: [
        {
          icon:
            'https://shouyiner-prod.oss-cn-beijing.aliyuncs.com/wxapp/shanqian/cost/seal_icon.png',
          name: '印章管理',
          hasAuth: true,
        },
        {
          icon:
            'https://shouyiner-prod.oss-cn-beijing.aliyuncs.com/wxapp/shanqian/cost/cost_icon.png',
          name: '费用管理',
          hasAuth: true,
        },
        {
          icon:
            'http://shouyiner-prod.oss-cn-beijing.aliyuncs.com/wxapp/shanqian/cost/invoice_apply_icon.png',
          name: '发票申请',
          hasAuth: true,
        },
        {
  ...
  onShow: function() {
    ...
    my.getAuthCode({
      ...
      success: (res) => {
        ...
        my.getAuthUserInfo({
          success: (userInfo) => {
            this.updateStateAttr({avatar: userInfo.avatar, userName: userInfo.nickName})
          },
        })
      },
    })
    ...
    if (
      (this.currentUser.companyId &&
        ...
        !this.currentUser.roleTypes.includes('1') &&
        !this.currentUser.roleTypes.includes('2')) ||
      (this.currentUser.companyId && this.currentUser.roleTypes == null) ||
      !this.hasLogin
    ) {
    ...
    if (
      (this.currentUser.companyId && this.currentUser.payStatus != 2) ||
      !this.currentUser.companyId
    ) {
  ...
  methods: {
    ...
    getWechatUserInfo(data) {
      ...
      put({
        ...
        success: () => {
          this.updateStateAttr({avatar: data.detail.avatarUrl})
        },
      })
    },
    ...
    handleSliderOperateFun(e) {
      switch (e.detail) {
        case 0:
          uni.navigateTo({
            url:
              '/pages/collection/templateCollection/templateCollection?isTemplate=1',
          })
          break
        ...
        case 1:
          uni.navigateTo({
            url:
              '/pages/collection/dirctCollection/dirctCollection?isTemplate=0',
          })
          break
