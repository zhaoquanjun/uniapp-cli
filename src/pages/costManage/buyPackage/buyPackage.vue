<template>
  <view class="buy-card--page">
    <view class="buy-amount--section">
      <view class="amount-list">
        <view class="list-area">
          <view
            :class="
              'list-item ' + (rechargeValue == item.price ? 'active' : '')
            "
          ...
          >
            <view class="item-title">
              <view class="item-desc"
                >{{ item.amount }}份合同（{{ item.setMealLife }}年有效期）</view
              >
        ...
        <checkbox-group @change="handleChangeSelectStatus">
          <view class="card way">
            <label class="flex-box">
              <view class="item-l">
                <image
                  class="card"
                  src="http://shouyiner-prod.oss-cn-beijing.aliyuncs.com/wxapp/shanqian/cost/shanqian_card.png"
                  mode="aspectFit"
                ></image>
              ...
              <view class="item-r">
                <text class="amount" v-if="checkedStatus.card"
                  >支付 {{ payInfo.card }}元</text
                >
                <checkbox
                  value="card"
                  color="#ed6c00"
                  :checked="checkedStatus.card"
                />
              </view>
            </label>
          </view>
          <view class="account way">
            <label class="flex-box">
              <view class="item-l">
                <image
                  src="http://shouyiner-prod.oss-cn-beijing.aliyuncs.com/wxapp/shanqian/cost/shanqian_icon.png"
                  mode="aspectFit"
                  alt=""
                />
              ...
              <view class="item-r">
                <text class="amount" v-if="checkedStatus.account"
                  >支付 {{ payInfo.account }}元</text
                >
                <checkbox
                  value="account"
                  color="#ed6c00"
                  :checked="checkedStatus.account"
                />
              </view>
            </label>
          </view>
          <view class="third way">
            <label class="flex-box">
              <view class="item-l">
                ...
                <image
                  class="wx"
                  src="http://shouyiner-prod.oss-cn-beijing.aliyuncs.com/wxapp/shanqian/cost/wx_icon.png"
                  mode="aspectFit"
                ></image>
                ...
                <image
                  class="wx"
                  src="http://shouyiner-prod.oss-cn-beijing.aliyuncs.com/wxapp/shanqian/cost/zfb_icon.png"
                  mode="aspectFit"
                ></image>
              ...
              <view class="item-r">
                <text class="amount" v-if="checkedStatus.third"
                  >支付 {{ payInfo.third }}元</text
                >
                <checkbox
                  value="third"
                  color="#ed6c00"
                  :checked="checkedStatus.third"
                />
              </view>
            </label>
          </view>
    ...
    <view
      :class="'confirm-btn ' + (!canNext ? 'not-allowed' : '')"
      @tap="_handleConfirmBuy"
      >确认购买</view
    >
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    _setCanNextValue() {
      ...
      if (
        this.total == 0 ||
        Number(this.payInfo.card) +
          Number(this.payInfo.account) +
          Number(this.payInfo.third) <
          Number(this.rechargeValue)
      ) {
        value = false
      } else {
        value = true
      ...
      get({
        ...
        success: (res) => {
          ...
          if (res.couponBalance && Number(res.couponBalance) > 0) {
            pay.card =
              Number(res.couponBalance) > Number(this.rechargeValue)
                ? Number(this.rechargeValue)
                : Number(res.couponBalance)
          }
          if (!res.couponBalance && Number(res.expenseBalance) > 0) {
            pay.account =
              Number(res.expenseBalance) > Number(this.rechargeValue)
                ? Number(this.rechargeValue)
                : Number(res.expenseBalance)
          }
          this.accountInfo = {
      ...
      get({
        ...
        success: (res) => {
          ...
          res.map((it) => {
            ...
            if (it.originPrice)
              it.originPrice = Number((it.originPrice / 100).toFixed(2))
          })
          this.packages = res
      ...
      if (
        ...
        if (
          ...
          if (Number(this.rechargeValue) > Number(this.accountInfo.card)) {
            o.account =
              Number(this.rechargeValue) - Number(this.accountInfo.card) >
          ...
          o.account =
            Number(this.accountInfo.account) > Number(this.rechargeValue)
              ? Number(this.rechargeValue)
              : Number(this.accountInfo.account)
        }
      }
      ...
      if (
        ...
        if (
          ...
          if (Number(o.third) - Number(this.accountInfo.account) > 0) {
            o.third = Number(
              (Number(o.third) - Number(this.accountInfo.account)).toFixed(2)
            )
          } else {
            o.third = 0
          }
