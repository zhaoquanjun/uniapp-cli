...
<view class="pageContent">
	<view class="item">
		...
		<view class="item-Content">
			<view class="message">
				<text>经办人姓名</text>
				<input type="text" :value="userName" @input="bindKeyInput" placeholder="请输入" data-field="userName"></input>
			</view>
...
export default {
  ...
  methods: {
    ...
    hasRepeatCompany() {
      let company = uni.getStorageSync('linkMans');
      let flag = true;
    ...
    next() {
      ...
      Promise.all([promise1, promise2]).then(res => {
        ...
        let links = uni.getStorageSync('linkMans') || [];
        links.push(item);
